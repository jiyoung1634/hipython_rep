    ...
    while True:
        ...
        if res is not None and res.__len__() > 0:
            for record in res:
                ...
                full_contents = soup.select_one("#topic_contents")
                if full_contents is None:
                    cur.execute(ready_update_sql, ('5', ready_seq_no))
                    conn.commit()
                    print('[debug] content parse error --> break')
                    continue
                else:
                    full_contents = full_contents.get_text().strip()

                comments = soup.select("div.commentTD")
                tmp = []
                for comment in comments:
                    tmp.append(comment.get_text().strip())
                news_comments = "|".join(tmp)
                print('comments : ', news_comments)

                # insert
                bm_insert_sql = 'insert into gn_master (news_title, news_desc, news_comments, news_url, full_contents, news_update) values (?,?,?,?,?,now())'
                cur.execute(bm_insert_sql, (news_title, news_desc, news_comments, ready_source_url, full_contents))
                conn.commit()
