...
st.checkbox('체크하시오', on_change=checkbox_write)
...
menus = st.multiselect(
    '먹고싶은 거 다 골라', ['김밥', '떡볶이', '우동', '쫄면']
)
st.text(f'내가 선택한 메뉴는 {menus}')

# slider
score = st.slider('내 점수 선택', 0, 100, 10)   # 시작, 끝, 초기값
st.text(f'score : {score}')

from datetime import time
st_time, end_time = st.slider(
    '공부시간 선택',
    min_value=time(0), max_value=time(11), value=(time(8), time(18)),
    format = 'HH:mm'
)
...
txt2 = st.text_input('비밀번호', placeholder='비번을 입력하세요', type='password')
st.text(f'텍스트 입력 결과 : {txt1}, {txt2}')

# 파일업로더
# 업로드한 파일은 사용자의 세션에 있다. 즉 화면을 갱신하면 사라진다.
# 서버에 저장하려면 별도로 구현해야 한다.
# 데이터베이스에 저장하는 로직도 구현할 수 있다.
